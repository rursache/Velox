...
- App scan roots are rebuilt every index pass. Include `/Applications` on local non-boot volumes only. Skip network volumes before touching disk. Volumes are enumerated without `skipHiddenVolumes` so `nobrowse` disks count, but a hidden volume only qualifies when mounted directly under `/Volumes` with a plain name, which keeps Time Machine snapshots, simulator runtimes, and cryptexes out. Missing folders are a no-op. Volume mount/unmount/rename triggers a rebuild. Directory watchers on user-writable Applications folders debounce a rebuild so App Store installs show up. Rebuilds single-flight and skip the icon cache wipe when the app set is unchanged. A 2-minute timer and a same-age stale check when the panel opens are fallbacks. Search drops `/Volumes/` hits whose files are gone so ejected disks do not linger
