...
- App scan roots are rebuilt every index pass. Include `/Applications` on local non-boot volumes only. Xcode nests apps inside its own bundle (Icon Composer, Instruments, Create ML) and the walker never descends into a `.app`, so every registered Xcode's `Contents/Applications` is added as a root. Nested roots are never watched, because replacing Xcode already shows up in the parent Applications folder, so the folder watcher asks for scan roots without them and skips that lookup. Skip network volumes before touching disk. Volumes are enumerated without `skipHiddenVolumes` so `nobrowse` disks count, but a hidden volume only qualifies when mounted directly under `/Volumes` with a plain name, which keeps Time Machine snapshots, simulator runtimes, and cryptexes out. Missing folders are a no-op. Volume mount/unmount/rename triggers a rebuild. Directory watchers on user-writable Applications folders debounce a rebuild so App Store installs show up. Rebuilds single-flight and skip the icon cache wipe when the app set is unchanged. A 2-minute timer and a same-age stale check when the panel opens are fallbacks. Search drops `/Volumes/` hits whose files are gone so ejected disks do not linger
