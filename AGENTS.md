...
- App scan roots are rebuilt every index pass. Include `/Applications` on local non-boot volumes only. Skip network volumes before touching disk. Missing folders are a no-op. Volume mount/unmount/rename triggers a rebuild. Directory watchers on those Applications folders debounce a rebuild so App Store installs show up. A 2-minute timer and a stale check when the panel opens are fallbacks. Search drops `/Volumes/` hits whose files are gone so ejected disks do not linger
